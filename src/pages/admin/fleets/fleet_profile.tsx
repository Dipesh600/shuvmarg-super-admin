    ...
    if (!fleet) {
        return (
            <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
                <XCircle className="h-16 w-16 text-destructive" />
                <h2 className="text-2xl font-bold">Fleet record is unavailable</h2>
                <Button onClick={() => navigate("/admin/fleets")}>Back to Fleets</Button>
            </div>
        );
    }
    const selectedAmenities: Array<string | FleetAmenity> = Array.isArray(fleet?.vehicle?.features)
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-8">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                        <h2 className="text-3xl font-black tracking-tighter truncate">{fleet.vehicle.busName}</h2>
                    ...
                    <p className="text-muted-foreground mt-1 flex items-center gap-2 text-sm tracking-tighter">
                        ...
                        {fleet.vehicle.busNumber} • {fleet.fleetCode || fleet.fleetId}
                    </p>
                </div>
                <div className="flex flex-wrap gap-2 w-full md:w-auto">
                    ...
                    <SuspendDialog
                        entityType="bus"
                        entityName={fleet.vehicle.busName}
                    ...
                    <Card className="border-t-4 border-t-primary shadow-lg overflow-hidden group">
                        ...
                        <CardContent className="pt-8 space-y-8">
                            <div className="flex flex-col items-center text-center space-y-4">
                                ...
                                <div>
                                    <h3 className="text-2xl font-black tracking-tighter">{fleet.vehicle.busNumber}</h3>
                                    <p className="text-muted-foreground font-medium uppercase text-xs tracking-widest mt-1 opacity-70 italic">{fleet.vehicle.busType} CATEGORY</p>
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <Badge variant="outline" className="rounded-full border-muted-foreground/30 px-3 py-1 text-[10px] uppercase tracking-tighter">
                                        YEAR {fleet.vehicle.registrationYear || "N/A"}
                                    </Badge>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="p-4 bg-muted/20 rounded-2xl border border-dashed border-muted-foreground/20 text-center">
                                    ...
                                    <p className="text-3xl font-black text-primary">{fleet.vehicle.totalSeats}</p>
                                </div>
                                <div className="p-4 bg-muted/20 rounded-2xl border border-dashed border-muted-foreground/20 text-center">
                                    <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest mb-1 opacity-60">Layout</p>
                                    <p className="text-3xl font-black text-primary uppercase">{typeof fleet.seatLayout === "object" && fleet.seatLayout?.assigned ? "Assigned" : "Pending"}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                            ...
                            <div className="flex flex-col gap-5">
                                <div className="flex items-start gap-4 p-4 rounded-xl bg-muted/30 border border-muted/60">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-black text-lg leading-none tracking-tighter">{fleet.owner.companyName || fleet.owner.ownerName}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 px-2">
                                    <div className="flex items-center gap-4 group">
                                        ...
                                        <div>
                                            ...
                                            <p className="font-bold text-sm truncate max-w-[200px]">{fleet.owner.email || "N/A"}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4 group">
                                        <div className="h-8 w-8 rounded-full bg-primary/5 flex items-center justify-center border group-hover:bg-primary/10 transition-colors">
                                        ...
                                        <div>
                                            <p className="text-[10px] font-black uppercase text-muted-foreground opacity-50 tracking-widest">Phone Number</p>
                                            <p className="font-bold text-sm">{fleet.owner.phone || "N/A"}</p>
                                        </div>
                                    </div>
                                </div>
                                <Button variant="link" className="p-0 h-auto justify-start text-primary font-black text-xs uppercase transition-all hover:tracking-widest" onClick={() => navigate(`/admin/busowners/${fleet.owner.ownerId}`)}>
                                    View Full History →
                                </Button>
                            </div>
                        </CardContent>
                ...
                <div className="lg:col-span-2 space-y-8">
                        ...
                        <Card className="shadow-lg border-primary/10">
                            ...
                            <CardContent>
                                <div className="grid grid-cols-2 gap-3">
                                    {(fleet.documents?.fleetImages?.count || 0) > 0 ? (
                                        Array.from({ length: fleet.documents?.fleetImages?.count || 0 }, (_, idx) => (
                                            <Button key={idx} variant="outline" className="relative aspect-[4/3] h-auto flex-col gap-2 rounded-2xl" onClick={() => setPhotoRequest({ fleetId: fleet.fleetId, slot: "fleetImages", imageIndex: idx })}>
                                                <Bus className="h-5 w-5" />
                                                <span className="text-xs font-bold">View photo {idx + 1}</span>
                                            </Button>
                                        ))
                                    ) : (
                            ...
                            <CardContent className="pt-2">
                                <div className="flex flex-wrap gap-3">
                                    {selectedAmenities.map((amenity, index) => (
                                        <div key={typeof amenity === "string" ? amenity : amenity.id || amenity._id || index} className="flex items-center gap-3 p-3 pr-5 bg-background border border-primary/5 rounded-2xl shadow-sm hover:shadow-md hover:border-primary/20 transition-all group cursor-default">
                    ...
                    <Card className="shadow-lg border-l-4 border-l-primary/60 border-primary/10">
                        <CardHeader className="border-b border-white/5 bg-white/5 pb-4">
                            ...
                            <CardDescription className="text-xs uppercase tracking-widest font-medium opacity-50 italic">
                                Reviewed stops for this route
                            </CardDescription>
                        </CardHeader>
                        <CardContent className="pt-4">
                            <div className="grid md:grid-cols-2 gap-6">
                                {fleet.route?.servedStops?.map((point, index) => (
                                    <div key={point.stopId || `${point.name}-${index}`} className="relative group p-6 rounded-3xl bg-muted/10 border border-muted/50 hover:bg-muted/20 hover:border-primary/20 transition-all overflow-hidden flex items-start gap-4">
                                        ...
                                        <div className="space-y-4 flex-1 min-w-0">
                                            <div>
                                                <p className="font-black text-xl tracking-tighter truncate">{point.name || "Unnamed stop"}</p>
                                                <div className="flex items-center gap-2 mt-1">
                                                     <Badge variant="outline" className="text-[9px] px-2 py-0 border-primary/30 text-primary uppercase font-black tracking-widest h-5">{point.usage || "STOP"}</Badge>
                                                     <p className="text-xs font-bold text-muted-foreground truncate italic">Reviewed route</p>
                                                </div>
                                            </div>
                                            <div className="flex flex-wrap gap-x-6 gap-y-2 pt-2 border-t border-muted/80">
                                                <div className="flex flex-col">
                                                    ...
                                                    <span className="font-bold text-xs">Managed in route setup</span>
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-[8px] font-black uppercase text-muted-foreground opacity-60">Coordinates</span>
                                                    <span className="text-[10px] text-primary/80">Registry protected</span>
                                                </div>
                                            </div>
                                        </div>
                            ...
                            <div className="mt-8 flex items-center gap-3 p-4 bg-primary/5 rounded-2xl border border-primary/10">
                                ...
                                <p className="text-xs font-bold text-muted-foreground tracking-tight italic">
                                    {fleet.route?.origin && fleet.route?.destination ? `${fleet.route.origin} → ${fleet.route.destination}` : "Route setup is not configured yet."}
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="shadow-lg h-32 flex items-center justify-center opacity-40 grayscale pointer-events-none overflow-hidden relative group">
                         <div className="text-center space-y-1 relative z-10 transition-all group-hover:scale-105">
                             ...
                             <p className="text-sm font-black uppercase tracking-[0.2em]">No Recent Analytics</p>
