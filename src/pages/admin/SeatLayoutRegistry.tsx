...
export default function SeatLayoutRegistry() {
  ...
  async function reload(select = selectedId) {
    ...
    if (id) { const value = await getSeatLayoutTemplate(id); setDetail(value); setName(value.template.name); setCategory(value.template.vehicleCategory); setLayout(value.revisions.find((revision) => revision.layout)?.layout || null); }
  }
  useEffect(() => {
    let active = true;
    void Promise.all([listSeatLayoutTemplates(), listSeatLayoutChangeRequests()])
      .then(async ([items, pending]) => {
        ...
        if (!active) return;
        setDetail(value); setName(value.template.name);
  ...
  async function select(id: string) { setSelectedId(id); const value = await getSeatLayoutTemplate(id); setDetail(value); setName(value.template.name); setCategory(value.template.vehicleCategory); setLayout(value.revisions.find((revision) => revision.layout)?.layout || null); }
  async function saveDraft(value: SeatLayoutV3) {
    ...
    try {
      ...
      if (!templateId) {
        const created = await createSeatLayoutTemplate({ name: normalizedName, vehicleCategory: category });
  ...
  return <div className="space-y-6 pb-12">
    ...
    <div className="grid gap-5 xl:grid-cols-[280px_minmax(0,1fr)]"><aside className="rounded-3xl border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center justify-between px-3 py-3"><span className="text-xs font-black uppercase tracking-widest text-white/40">Library</span><Library className="size-4 text-[#D3D925]" /></div>{templates.map((template) => <button key={template.id} onClick={() => void select(template.id)} className={`mb-2 w-full rounded-2xl border p-4 text-left ${selectedId === template.id ? "border-[#D3D925]/60 bg-[#D3D925]/10" : "border-white/5 hover:bg-white/5"}`}><div className="flex items-start justify-between gap-2"><strong className="text-sm text-white">{template.name}</strong><ChevronRight className="size-4 text-white/30" /></div><p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-white/35">{template.templateCode} · {template.currentPublishedRevisionId ? "Published" : "Draft only"}</p></button>)}</aside>
      <main className="space-y-5"><section className="grid gap-4 rounded-3xl border border-white/10 bg-white/[0.025] p-5 md:grid-cols-4"><label className="text-[10px] font-bold uppercase tracking-widest text-white/40 md:col-span-3">Template name<Input value={name || detail?.template.name || ""} onChange={(event) => setName(event.target.value)} placeholder="e.g. Deluxe 2 × 1" className="mt-2 border-white/10 bg-black/20" /></label><label className="text-[10px] font-bold uppercase tracking-widest text-white/40">Vehicle<select value={detail?.template.vehicleCategory || category} disabled={Boolean(detail)} onChange={(event) => setCategory(event.target.value as VehicleCategory)} className="mt-2 h-10 w-full rounded-md border border-white/10 bg-[#111] px-3 text-sm"><option>BUS</option><option>MINIBUS</option><option>HIACE</option></select></label><div className="md:col-span-4 rounded-xl border border-white/10 bg-black/20 px-3 py-2 text-xs text-white/45">{detail ? `Registry code: ${detail.template.templateCode}` : `Registry code will be generated automatically as ${category}-0001, ${category}-0002…`}</div><label className="text-[10px] font-bold uppercase tracking-widest text-white/40 md:col-span-4">Revision note<Input value={summary} onChange={(event) => setSummary(event.target.value)} className="mt-2 border-white/10 bg-black/20" /></label></section>
