...
const AgentLeads = () => {
  ...
  const { data, isLoading, isError, refetch, isFetching } = useQuery({
    queryKey: ["agentLeads", leadTypeFilter, statusFilter, debouncedSearch, page],
    queryFn: () =>
      getAgentLeads({
  ...
  if (isError) {
    return (
      <div className="text-white/60 py-12 text-center">
        Failed to load agent leads. Please refresh.
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 mb-6">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-white">Agent Leads</h2>
          <p className="text-white/60 mt-1 font-medium text-sm">
            Prospects who expressed interest in becoming a Shuv Marg agent partner
          </p>
        </div>
        <button
          onClick={() => refetch()}
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-white/60 font-medium">No agent leads found</p>
              <p className="text-white/30 text-sm mt-1">
                {leadTypeFilter !== "all" || statusFilter !== "all" || debouncedSearch
                  ? "Try clearing the filters."
                  : "Leads appear when someone starts agent registration or submits a contact form."}
              </p>
            </div>
          ) : (
            <>
