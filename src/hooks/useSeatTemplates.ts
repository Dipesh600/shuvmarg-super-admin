...
export const useCreateSeatTemplate = () => {
  ...
  return useMutation({
    mutationFn: (payload: CreateTemplatePayload) => createTemplateForOwner(payload),
    onSuccess: () => {
...
export const useUpdateSeatTemplate = (id: string) => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: UpdateTemplatePayload) => updateSeatTemplate(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seatTemplates"] });
...
export const useToggleSeatTemplateStatus = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["seatTemplates"] });
      toast.success("Seat template status updated");
    },
    onError: (error: any) => {
