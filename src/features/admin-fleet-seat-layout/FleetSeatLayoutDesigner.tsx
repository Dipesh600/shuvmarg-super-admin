...
export default function FleetSeatLayoutDesigner({ title, layout, busy, onChange, onUse, onCancel }: Props) {
  return <div className="space-y-4">
    <div className="flex items-center gap-3">
      <button type="button" onClick={onCancel} className="flex size-9 items-center justify-center rounded-xl border border-border" aria-label="Back to templates"><ArrowLeft className="size-4" /></button>
      <div><h4 className="font-black text-foreground">{title}</h4><p className="text-xs text-muted-foreground">Add, remove, move or rename only what differs on this bus.</p></div>
    </div>
