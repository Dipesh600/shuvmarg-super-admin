...
function Deck({ section, layout, tool, selectedId, onChange, onSelect }: {
  ...
  return <section className="min-w-0 rounded-3xl border border-white/10 bg-[#0d1010] p-4 shadow-2xl">
    ...
    <div className="mx-auto grid max-w-[360px] gap-1.5 rounded-[2rem] border-2 border-white/10 bg-black/25 p-3"
      ...
      {cells.map(({ x, y }) => <button key={`${x}:${y}`} type="button" aria-label={`Grid ${x + 1}, ${y + 1}`}
      ...
      {section.elements.map((element) => <button key={element.elementId} type="button" onClick={() => tool === "SELECT" ? onSelect(element.elementId) : onChange(applyTool(layout, section.sectionId, element.position.x, element.position.y, tool))}
        className={cn("z-10 flex min-w-0 items-center justify-center gap-1 rounded-lg border text-[10px] font-black shadow-lg transition hover:brightness-125", colors[element.kind], selectedId === element.elementId && "ring-2 ring-[#D3D925] ring-offset-2 ring-offset-black")}
