...
export function CorridorWorkspace() {
  ...
  return (
    <div className="space-y-5">
      ...
      <VariantDetailsDialog variant={viewingVariant} open={Boolean(viewingVariant)} allRegistryStops={stops} onOpenChange={(open) => !open && setViewingVariant(null)} onRevise={(variant) => reviseVariantMutation.mutate(variant._id)} onResume={(variant) => { setViewingVariant(null); resumeVariantDraft(variant); }} onActivate={(variant) => activateVariantMutation.mutate(variant._id)} onStopsUpdated={() => { void queryClient.invalidateQueries({ queryKey: ["corridor-variants", selectedCorridorId] }); void queryClient.invalidateQueries({ queryKey: ["route-variant-details", viewingVariant?._id] }); }} />
