...
export function CorridorWorkspace() {
  ...
  return (
    <div className="space-y-5">
      ...
      <VariantDetailsDialog variant={viewingVariant} open={Boolean(viewingVariant)} onOpenChange={(open) => !open && setViewingVariant(null)} onRevise={(variant) => reviseVariantMutation.mutate(variant._id)} onResume={(variant) => { setViewingVariant(null); resumeVariantDraft(variant); }} onActivate={(variant) => activateVariantMutation.mutate(variant._id)} />
