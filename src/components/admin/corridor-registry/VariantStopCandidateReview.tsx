...
function candidateSource(candidate: VariantStopCandidate) {
  ...
  if (candidate.classification?.entityType === "SERVICE_AREA") {
    const transitBacked = candidate.classification.reasonCodes.includes("TRANSIT_EVIDENCE_CORROBORATED");
    return transitBacked
      ? "Repeated on-path locality corroborated by nearby transit evidence — admin confirmation required"
      : "Repeated passenger locality observed along the selected road — admin confirmation required";
  }
